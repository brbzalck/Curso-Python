...
data_emprestimo = datetime(2020, 12, 20)  # Criando uma data simples com datetime(ano, mes, dia) # noqa 501
cinco_anos = relativedelta(years=5)  # relativedelta para armazenar 5 anos numa VAR # noqa 501
...
valor_parcelas = valor_total / numero_parcelas  # Dividindo valor total pelos meses de pagamento # noqa 501

for data in data_parcelas:  # Exibindo cada parcela formatada bunitinha
    print(data.strftime('%d/%m/%Y'), f'R${valor_parcelas:,.2f}')  # formatação float , . 00 # noqa 501
    # Usando a função strftime na nossa data, para exibir no nosso formato br dia/mes/ano # noqa 501
